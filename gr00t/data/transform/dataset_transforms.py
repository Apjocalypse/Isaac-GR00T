...
class EpisodeProcessorLoad:
    ...
    def __call__(self, episode_info):
        state_path = os.path.join(episode_info['meta_info']["episode_dir"], "aligned_joints.h5")
        ...
        meta_path = os.path.join(episode_info['meta_info']["episode_dir"], "meta_info.json")
        with open(meta_path, "r") as fid:
            meta_info = json.load(fid)
        data_path = os.path.join(episode_info['meta_info']["episode_dir"], "data_info.json")
        ...
        data_info["cam_cfg"] = cam_cfg
        episode_info["meta_info"].update(data_info)
...
class EpisodeProcessorRelableStaticFrames:
    ...
    def __call__(self, episode_info):
        ...
        start_idx = episode_info["frame_infos"][0]['frame_idx']
        end_idx = episode_info["frame_infos"][-1]['frame_idx']

        if episode_info['meta_info']["label_info"]["action_config"][0]["start_frame"] < start_idx:
            episode_info['meta_info']["label_info"]["action_config"][0]["start_frame"] = start_idx
        if episode_info['meta_info']["label_info"]["action_config"][-1]["end_frame"] > end_idx:
            episode_info['meta_info']["label_info"]["action_config"][-1]["end_frame"] = end_idx
        return episode_info
...
class DatasetTargetDualArmChunk:
    ...
    def mp_worker(self, info):
        ...
        try:
            ...
            # task_specific_cfg = meta_info["task_specific_cfg"]
            action_config = meta_info["label_info"]["action_config"]
            ...
            for act_step in action_config:
                ...
                job_description = meta_info["english_task_name"]
                ...
                for current_idx in range(start_idx, end_idx):
                    ...
                    used_cam_cfg = meta_info['cam_cfg']

                    model_target = {
                        "task_id": f"{meta_info['task_id']}",
                        "job_id": f"{meta_info['job_id']}",
                        "sn_code": f"{meta_info['sn_code']}",
                        "episode_id": f"{meta_info['episode_id']}",
                        "episode_dir": meta_info["episode_dir"],
                        ...
                        "timestamp": info["frame_infos"][current_idx - start_idx]['timestamp']
                    }

                    results.append(model_target)
        except Exception as error:
            logger.error(f'{meta_info["episode_dir"]} met error: {error}')
            results = None
        return results
    ...
    def __call__(self, inputs):
        ...
        for key in keys:
            if self.source_key in key:
                img = inputs.pop(key).resize(self.size)
                unsqueezed_videos.append(np.expand_dims(np.array(img, dtype=np.uint8), axis = -4)) # [..., H, W, C] -> [..., 1, H, W, C]
        inputs['videos'] =  np.concatenate(unsqueezed_videos, axis=-4) # [V, H, W, C]
        return inputs
...
from gr00t.utils.constants import (CLIP_MEAN, CLIP_STD, IMAGENET_MEAN, IMAGENET_STD,
