...
from gr00t.data.alpha_base_cfg import BaseDatasetArguments, BaseDataTrainingArguments, BaseModelArguments
...
class ActionSpacePadder():
    ...
    def get_action(cls, targets, chunk_size=1):
        ...
        mask = np.zeros((chunk_size, cls.action_space_len), dtype=np.float32)
        for key, value in targets.items():
            if key in cls.action_space:
                value = np.array(value)
                start_idx, max_idx = cls.action_space[key]
                if value.shape[-1] <= (max_idx - start_idx) and value.shape[0] >= chunk_size:
                    action[:, start_idx : start_idx + value.shape[-1]] = value[:chunk_size]
                    mask[:, start_idx : start_idx + value.shape[-1]] = np.ones_like(value[:chunk_size])
                else:
                    raise ValueError(
...
class AlphaTrainingArguments(TrainingArguments):
    ...
    label_names: List[str] = field(default_factory=lambda: ["action_gts"]) # type: ignore
